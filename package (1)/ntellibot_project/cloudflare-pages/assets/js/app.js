...
async function fetchNewsData() {
    try {
        ...
        const [tier1Response, tier2Response] = await Promise.all([
            fetchFromNetlifyFunction('newsTier1', {}, controller.signal),
            fetchFromNetlifyFunction('newsTier2', {}, controller.signal)
        ]);
...
async function generateMilitaryActivityZones() {
    try {
        ...
        const [tier1Response, tier2Response] = await Promise.all([
            fetchFromNetlifyFunction('newsTier1'),
            fetchFromNetlifyFunction('newsTier2')
        ]);
