...
const API_CONFIG = {
    ...
    news: {
        // Use Netlify function for RSS feeds
        endpoint: '/api/news',
        timeout: 30000
    },
    events: {
...
// News API - RSS feeds via Netlify Function
async function fetchNewsData() {
    try {
        const controller = new AbortController();
        const timeoutId = setTimeout(() => controller.abort(), 30000);
        
        const response = await fetchFromNetlifyFunction('news', {}, controller.signal);
        clearTimeout(timeoutId);
        
        if (response && response.news && Array.isArray(response.news)) {
            let allNews = response.news;
...
async function generateMilitaryActivityZones() {
    try {
        ...
        // Get events from both news and events APIs for comprehensive coverage
        const eventsResponse = await fetchFromNetlifyFunction('events');
        const newsResponse = await fetchFromNetlifyFunction('news');
        ...
        if (newsResponse && newsResponse.news && Array.isArray(newsResponse.news)) {
            allEvents = allEvents.concat(newsResponse.news.map(n => ({
                title: n.title,
                summary: n.summary,
                source: n.source,
                pubDate: n.pubDate,
