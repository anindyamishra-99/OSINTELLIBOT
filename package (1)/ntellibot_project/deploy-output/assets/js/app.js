...
// API Configuration - Use Netlify Functions for all API calls (CORS-compatible)
const API_CONFIG = {
    crypto: {
        // Use Netlify function for CoinGecko
        endpoint: '/api/crypto',
        timeout: 15000
    },
    forex: {
        // Use Netlify function for Frankfurter
        endpoint: '/api/forex',
        timeout: 15000
    },
    news_tier1: {
        // Tier 1 RSS feeds (16 feeds)
        endpoint: '/api/news-tier1',
        timeout: 55000
    },
    news_tier2: {
        // Tier 2 RSS feeds (16 feeds)
        endpoint: '/api/news-tier2',
        timeout: 55000
    },
    events: {
        // Use Netlify function for events
        endpoint: '/api/events',
        timeout: 30000
    },
    signals: {
        // Use Netlify function for signals
        endpoint: '/api/signals',
        timeout: 30000
    },
    predictions: {
        // Use Netlify function for predictions
        endpoint: '/api/predictions',
        timeout: 30000
    },
    military: {
        // Use GDELT via Netlify function for military activity
...
async function fetchFromAPI(url, options = {}) {
    try {
        // Remove AbortController - let browser handle timeouts naturally
        // The backend has proper timeout handling (6s per feed, 55s overall)
        
        const response = await fetch(url, {
            ...options
            // No signal - backend handles its own timeouts
        });
        ...
        if (error.name === 'AbortError' || error.message.includes('aborted') || error.message.includes('abort')) {
            console.warn(`[API] Aborted: ${url} - Server took too long (using cached data if available)`);
            return null; // Will use cached data in the calling function
        }
...
// Crypto API - CoinGecko via Netlify Function
async function fetchCryptoData() {
    try {
        const response = await fetchFromNetlifyFunction('crypto');
...
// Forex API - Frankfurter via Netlify Function
async function fetchForexData() {
    try {
        const response = await fetchFromNetlifyFunction('forex');
...
// News API - RSS feeds via parallel tier1 + tier2 endpoints (frontend-side aggregation)
async function fetchNewsData() {
    try {
        console.log('[News Debug] Starting parallel news fetch (tier1 + tier2)...');
        
        const startTime = Date.now();
        
        // Fetch both tiers in parallel directly from frontend
        const [tier1Response, tier2Response] = await Promise.all([
            fetchFromAPI(`${API_CONFIG.news_tier1.endpoint}?t=${Date.now()}`, { timeout: API_CONFIG.news_tier1.timeout }),
            fetchFromAPI(`${API_CONFIG.news_tier2.endpoint}?t=${Date.now()}`, { timeout: API_CONFIG.news_tier2.timeout })
        ]);
        
        const tier1Time = Date.now();
        console.log(`[News Debug] Tier1 fetch completed in ${tier1Time - startTime}ms`);
        
        const tier2Time = Date.now();
        console.log(`[News Debug] Tier2 fetch completed in ${tier2Time - tier1Time}ms`);
        
        // Collect all news articles from both tiers
        let allNews = [];
        let sources = new Set();
        ...
        if (tier1Response && tier1Response.articles && Array.isArray(tier1Response.articles)) {
            ...
            tier1Response.articles.forEach(item => {
                if (item.source) sources.add(item.source);
            });
            tier1Success = true;
            console.log(`[News Debug] Tier1: ${tier1Response.articles.length} articles`);
        } else {
            console.warn('[News Debug] Tier1 response invalid or empty');
        }
        ...
        if (tier2Response && tier2Response.articles && Array.isArray(tier2Response.articles)) {
            ...
            tier2Response.articles.forEach(item => {
                if (item.source) sources.add(item.source);
            });
            tier2Success = true;
            console.log(`[News Debug] Tier2: ${tier2Response.articles.length} articles`);
        } else {
            console.warn('[News Debug] Tier2 response invalid or empty');
        }
        
        console.log(`[News Debug] Total articles before dedup: ${allNews.length}`);
        console.log(`[News Debug] Sources found: ${sources.size}`);
        ...
        console.log(`[News Debug] Total articles after dedup: ${allNews.length}`);
        ...
        console.log('[News Debug] After 7-day filter:', allNews.length);
        
        // Store final result
        appState.data.news = allNews.slice(0, 50);
        
        const totalFetchTime = Date.now() - startTime;
        console.log(`[News Debug] Total fetch time: ${totalFetchTime}ms`);
        
        // Determine overall status based on tier responses
        let overallStatus = 'offline';
        let statusMessage = 'Unable to fetch news';
        
        if (tier1Success && tier2Success) {
            overallStatus = 'online';
            statusMessage = 'All sources active';
        } else if (tier1Success || tier2Success) {
            overallStatus = 'partial';
            statusMessage = tier1Success ? 'Tier2 unavailable' : 'Tier1 unavailable';
        }
        ...
        appState.apiHealth.news = { 
            ...
            tier1Status: tier1Success ? 'online' : 'offline',
            tier2Status: tier2Success ? 'online' : 'offline',
            message: statusMessage
        };
        
        console.log('[News Debug] Final news count:', appState.data.news.length);
        ...
        if (error.name === 'AbortError' || error.message.includes('aborted')) {
            console.warn('[News Debug] Request aborted (timeout):', error.message);
            
            if (appState.data.news && appState.data.news.length > 0) {
                appState.apiHealth.news = { 
                    status: 'partial', 
                    sources: appState.data.news.length,
                    articles: appState.data.news.length,
                    error: 'Request timed out - using cached data',
                    message: 'Refresh to get latest data'
                };
                return appState.data.news;
            }
        }
        ...
        appState.apiHealth.news = { 
            status: 'offline', 
            error: error.message,
            message: error.name === 'AbortError' ? 'Request timed out - try refreshing' : 'Unable to fetch news'
        };
        return [];
    }
}

// Events API - GDELT + Wikipedia via Netlify Function
async function fetchEventsData() {
    try {
        const response = await fetchFromNetlifyFunction('events');
...
// Signals API - Real OSINT signals via Netlify Function
async function fetchSignalsData() {
    try {
        const response = await fetchFromNetlifyFunction('signals');
...
// Predictions API - Forecasts via Netlify Function
async function fetchPredictionsData() {
    try {
        const response = await fetchFromNetlifyFunction('predictions');
...
async function generateMilitaryActivityZones() {
    try {
        ...
        const eventsResponse = await fetchFromNetlifyFunction('events');
        
        // Get news from both tier endpoints in parallel (new frontend aggregation approach)
        let tier1News = null;
        let tier2News = null;
        
        try {
            [tier1News, tier2News] = await Promise.all([
                fetchFromAPI(`${API_CONFIG.news_tier1.endpoint}?t=${Date.now()}`, { timeout: API_CONFIG.news_tier1.timeout }),
                fetchFromAPI(`${API_CONFIG.news_tier2.endpoint}?t=${Date.now()}`, { timeout: API_CONFIG.news_tier2.timeout })
            ]);
        } catch (error) {
            console.warn('[Military Activity] Error fetching news tiers:', error.message);
        }
        ...
        // Combine news from both tiers
        let allNews = [];
        if (tier1News && tier1News.articles && Array.isArray(tier1News.articles)) {
            allNews = allNews.concat(tier1News.articles);
        }
        if (tier2News && tier2News.articles && Array.isArray(tier2News.articles)) {
            allNews = allNews.concat(tier2News.articles);
        }
        
        if (allNews.length > 0) {
            allEvents = allEvents.concat(allNews.map(n => ({
                title: n.title,
                summary: n.summary,
                source: n.source,
                pubDate: n.pubDate,
...
// Helper function to categorize military activity from news
...
function updateMilitaryHeatmap(map) {
    ...
    // Clear the events source to show only military activity on the map
...
// Add events source to map - only high/critical severity events
function addEventMarkers(map, events) {
    if (!map || !map.getSource('events')) return;
    
    // Filter to only high and critical severity events
    const filteredEvents = (events || []).filter(event => {
        const severity = event.severity || event.metadata?.severity || 'low';
        return severity === 'high' || severity === 'critical';
    });
    
    console.log(`[Map] Showing ${filteredEvents.length} high/critical events out of ${events?.length || 0} total events`);
...
function renderStats() {
    ...
    // Calculate last 7 days events for military activity
    ...
    const highCritical = last7d.filter(e => 
    ...
    if (totalEventsEl) totalEventsEl.textContent = last7d.length || events.length || '0';
...
function renderRecentEvents() {
    ...
    // Filter out irrelevant content and apply relevance filtering
    allEvents = allEvents.filter(event => {
        ...
        const contentFilters = [
            ...
            /earnings|revenue|profit|loss|stock price|quarterly|annual report|ceo|founder|executive/i,
        ...
        // Exclude very old events (older than 10 days)
        const eventDate = new Date(event.pubDate || 0);
        const tenDaysAgo = Date.now() - (10 * 24 * 60 * 60 * 1000);
        if (eventDate.getTime() < tenDaysAgo) return false;
        
        // Must have at least medium severity or significant geo-relevance
        const relevanceScore = event.geoRelevance?.score || 0;
        const validSeverities = ['critical', 'high', 'medium'];
        if (!validSeverities.includes(event.severity) && relevanceScore < 12) return false;
        ...
        <div class="military-activity-item ${zone.category || 'combined'}">
            ...
            <div class="military-activity-content">
                ...
                <div class="military-activity-footer">
                    <span class="military-activity-source">${zone.dataSource || 'Real-time GDELT Analysis'}</span>
        ...
        <tr>
            ...
            <td>
                <div style="display: flex; gap: 0.5rem;">
                    <a href="${source.url}" target="_blank" rel="noopener noreferrer" class="action-btn small" title="Open Source">Open</a>
